...
function tenderLines(f: ExtractedTenderFields): string[] {
  return [
    ...
    `  minExp   ${show(f.min_experience_years)} | ref letters ${show(f.reference_letters_required)} | within ${show(f.reference_letters_within_years)} yrs | min R${show(f.reference_letters_min_value)}`,
    ...(f.reference_letters_note ? f.reference_letters_note.split("\n").map((l) => `           ${l}`) : []),
